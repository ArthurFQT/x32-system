...
let ioOptionsCache = null;
function clamp(value, min, max) {
    return Math.max(min, Math.min(max, value));
}
function defaultBusOptions() {
...
function queryOscString(address) {
    return new Promise((resolve) => {
        ...
        const onMessage = (packet) => {
            try {
                const decoded = (0, osc_1.decodeOscMessage)(packet);
                if (decoded.address !== address) {
                    return;
                }
                const stringArg = decoded.args.find((arg) => typeof arg === "string");
                finish(stringArg);
            }
            catch {
        ...
        const requestPacket = (0, osc_1.encodeOscMessage)(address, []);
...
async function queryRealIoOptions() {
...
async function getIoOptions(forceRefresh) {
    if (!USE_REAL_X32_IO) {
...
const socket = (0, socket_io_client_1.io)(BACKEND_URL, {
...
socket.on("bridge:get-io-options", async (payload, callback) => {
...
socket.on("x32", (event) => {
    try {
        ...
        const message = (0, osc_1.encodeOscMessage)(command.path, [
            { type: command.argType, value: command.value },
        ]);
        udpClient.send(message, X32_PORT, X32_IP, (error) => {
            if (error) {
                console.error(`[BRIDGE][UDP_ERROR] token=${event.token} user=${event.user} path=${command.path} error=${error.message}`);
                return;
            }
            console.log(`[BRIDGE][SENT] token=${event.token} user=${event.user} path=${command.path} value=${command.value}`);
        });
    }
    catch (error) {
        console.error(`[BRIDGE][OSC_ERROR] token=${event.token} user=${event.user} error=${error instanceof Error ? error.message : "unknown"}`);
    }
});
udpClient.bind(() => {
